...
class Phase145HardeningTest extends TestCase
{
    ...
    public function test_expired_api_token_is_rejected(): void
    {
        ...
        $token = ApiToken::create(['user_id' => $user->id, 'name' => 'test', 'token_hash' => hash('sha256', 'expired-token'), 'expires_at' => Carbon::now()->subMinute()]);
        $this->withHeader('Authorization', 'Bearer expired-token')->getJson('/api/v1/auth/me')->assertUnauthorized();
        $this->assertNotNull($token->fresh()->revoked_at);
    }

    public function test_invalid_api_login_credentials_return_401(): void
    {
    ...
    {
        $this->postJson('/api/v1/auth/login', ['email' => 'missing@example.com', 'password' => 'wrong-password'])->assertUnauthorized();
    }

    public function test_public_product_api_hides_internal_product_state_fields_and_raw_stock(): void
    {
        $product = Product::factory()->create(['is_active' => true, 'stock' => 7]);
    ...
    {
        $user = User::factory()->create();
        ApiToken::create(['user_id' => $user->id, 'name' => 'test', 'token_hash' => hash('sha256', 'token'), 'expires_at' => Carbon::now()->addDay()]);
    ...
    public function test_oldest_active_api_tokens_are_revoked_when_limit_is_reached(): void
    {
        ...
        foreach (['token-1', 'token-2'] as $plainToken) {
            ApiToken::create(['user_id' => $user->id, 'name' => 'test', 'token_hash' => hash('sha256', $plainToken), 'expires_at' => Carbon::now()->addDay()]);
        }
