...
class ProcessBiteshipWebhook implements ShouldQueue
{
    ...
    {
        $event = BiteshipWebhookEvent::find($this->eventId);
        ...
        try {
            DB::transaction(function () use ($event) {
                ...
                $biteshipOrderId = $event->biteship_order_id;
                $order = Order::where('biteship_order_id', $biteshipOrderId)->lockForUpdate()->first();

                if (!$order) {
                    $event->forceFill([
                        'processed_at' => now(),
                        'processing_error' => 'Order not found; event acknowledged.',
                    ])->save();
                    return;
                }
                ...
                $normalizedStatus = is_string($status)
                    ? strtolower(preg_replace('/(?<!^)[A-Z]/', '_$0', $status))
                    : null;
                ...
                if (isset($updates['status']) && $updates['status'] !== $previousStatus) {
                    $order->recordStatusChange($updates['status'], 'biteship', 'Shipping webhook status update.');
                }
