...
class BiteshipShippingService
{
    ...
    private function client()
    {
        ...
        if (!$apiKey) throw new RuntimeException('BITESHIP_API_KEY belum dikonfigurasi.');
        return Http::withToken($apiKey)->acceptJson()->timeout((int) config('services.biteship.timeout', 10));
    }

    private function originPostalCode(): int
    {
        $postalCode = config('services.biteship.origin_postal_code');
        if (!$postalCode) throw new RuntimeException('BITESHIP_ORIGIN_POSTAL_CODE belum dikonfigurasi.');
    ...
    {
        if ($response->failed() || $response->json('success') === false) throw new RuntimeException($message);
    }
    ...
    private function mapCartItems(iterable $items): array
    {
        return collect($items)->map(function (CartItem $item): array {
            ...
            return [
                ...
                'value' => (int) round((float) ($variant?->price ?? $product->price)),
