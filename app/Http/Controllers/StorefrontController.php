...
class StorefrontController extends Controller
{
    ...
    public function products(Request $request): View { $term=$request->input('q',$request->input('search'));$products=Product::where('is_active',true)->with('category')->when($term,fn($query)=>$query->where(fn($q)=>$q->where('name','like','%'.$term.'%')->orWhere('sku','like','%'.$term.'%')))->latest()->paginate(12)->withQueryString();return view('store.products',compact('products')); }
