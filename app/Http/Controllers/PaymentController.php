    ...
    public function show(Order $order, PaymentGateway $gateway): View|\Illuminate\Http\RedirectResponse
    {
        ...
        if (!$token) {
            try {
                ...
                Log::error('Midtrans payment creation failed.', ['order_id' => $order->id, 'exception' => $e]);
                return redirect()->route('orders.show', $order)->with('error', 'Pembayaran sedang tidak tersedia. Silakan coba lagi beberapa saat lagi.');
            }
        }
        ...
        if ($midtransOrderNumber) {
            ...
            if ($order?->payment_status === 'paid' && str_starts_with($order->shipping_method, 'biteship:') && !$order->biteship_order_id) {
                try {
                    ...
                    Log::error('Automatic Biteship shipment creation after Midtrans payment failed.', ['order_id' => $order->id, 'exception' => $e]);
                }
            }
        }
